document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateFlowStep(step) {
    if (!step || !flowImage || !flowTitle || !flowText) return;

    flowSteps.forEach(item => item.classList.remove('active-step'));
    step.classList.add('active-step');

    flowImage.src = step.dataset.image || '';
    flowTitle.textContent = step.dataset.title || '';
    flowText.textContent = step.dataset.text || '';
  }

  flowSteps.forEach(step => {
    step.addEventListener('mouseenter', () => activateFlowStep(step));
    step.addEventListener('focus', () => activateFlowStep(step));
    step.addEventListener('click', () => activateFlowStep(step));
  });
