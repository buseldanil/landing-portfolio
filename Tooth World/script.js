document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateRoutineStep(step) {
    if (!step || !routineImage || !routineTitle || !routineText) return;

    routineSteps.forEach(item => item.classList.remove('active-step'));
    step.classList.add('active-step');

    routineImage.src = step.dataset.image || '';
    routineTitle.textContent = step.dataset.title || '';
    routineText.textContent = step.dataset.text || '';
  }

  routineSteps.forEach(step => {
    step.addEventListener('mouseenter', () => activateRoutineStep(step));
    step.addEventListener('focus', () => activateRoutineStep(step));
    step.addEventListener('click', () => activateRoutineStep(step));
  });
