...
function restartAutoplay() {
  ...
  autoplayTimer = setInterval(nextScreen, 4300);
}
...
tiltCards.forEach(card => {
  if (card.classList.contains('phone-main')) return;
  ...
  card.addEventListener('mousemove', event => {
    ...
    card.style.transform = `${originalTransform === 'none' ? '' : originalTransform} rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
  });
