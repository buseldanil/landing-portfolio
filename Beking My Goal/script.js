document.addEventListener('DOMContentLoaded', () => {
  ...
  const flowPreviewImage = document.getElementById('flowPreviewImage');
  const flowPreviewTitle = document.getElementById('flowPreviewTitle');
  const flowPreviewText = document.getElementById('flowPreviewText');

  function setActiveFlow(step) {
    if (!step || !flowPreviewImage || !flowPreviewTitle || !flowPreviewText) {
      return;
    }

    flowSteps.forEach(item => item.classList.remove('active-step'));
    step.classList.add('active-step');

    flowPreviewImage.src = step.dataset.image || '';
    flowPreviewTitle.textContent = step.dataset.title || '';
    flowPreviewText.textContent = step.dataset.text || '';
  }

  flowSteps.forEach(step => {
    step.addEventListener('mouseenter', () => setActiveFlow(step));
    step.addEventListener('focus', () => setActiveFlow(step));
    step.addEventListener('click', () => setActiveFlow(step));
  });
