document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateJourneyStep(step) {
    if (!step || !journeyImage || !journeyTitle || !journeyText) return;

    journeySteps.forEach(item => item.classList.remove('active-step'));
    step.classList.add('active-step');

    journeyImage.src = step.dataset.image || '';
    journeyTitle.textContent = step.dataset.title || '';
    journeyText.textContent = step.dataset.text || '';
  }

  journeySteps.forEach(step => {
    step.addEventListener('mouseenter', () => activateJourneyStep(step));
    step.addEventListener('focus', () => activateJourneyStep(step));
