document.addEventListener('DOMContentLoaded', () => {
  ...
  function activateExperienceStep(step) {
    if (!step || !experienceImage || !experienceTitle || !experienceText)
      return;

    experienceSteps.forEach(item => item.classList.remove('active-step'));
    step.classList.add('active-step');

    experienceImage.src = step.dataset.image || '';
    experienceTitle.textContent = step.dataset.title || '';
    experienceText.textContent = step.dataset.text || '';
  }

  experienceSteps.forEach(step => {
    step.addEventListener('mouseenter', () => activateExperienceStep(step));
    step.addEventListener('focus', () => activateExperienceStep(step));
    step.addEventListener('click', () => activateExperienceStep(step));
  });
