...
const updateActiveLink = () => {
  ...
  navLinks.forEach(link => {
    ...
    link.classList.toggle('active', href.includes(`#${currentId}`));
  });
};
...
const heroParallax = () => {
  ...
  if (mainPhone) mainPhone.style.transform = `translateY(${y * 0.04}px)`;
  if (sideLeft)
    sideLeft.style.transform = `translateY(${y * 0.08}px) rotate(-10deg)`;
  if (sideRight)
    sideRight.style.transform = `translateY(${y * -0.05}px) rotate(10deg)`;
  if (floatLeft) floatLeft.style.transform = `translateY(${y * 0.03}px)`;
  if (floatRight) floatRight.style.transform = `translateY(${y * -0.02}px)`;
};
